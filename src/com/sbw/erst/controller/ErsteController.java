...
public class ErsteController {
	...
	public String db(String command, String param1, String param2, String param3, String param4, String param5, Map<String, String> map) {
		...
		return "../DBOperation.getInstance()";
	}
	...
	public String registering(String email, String nickname, String password, Map<String, String> map) {
		if (email.length() > 64) {
			map.put("registerWarning", "您的邮箱太长，不能注册！");
			return "register/register";
		}
		if (nickname.length() > 64) {
			map.put("registerWarning", "您的昵称太长！");
			return "register/register";
		}
		if (password.length() > 16 || password.length() < 6) {
			map.put("registerWarning", "密码长度应在6到16位之间！");
			return "register/register";
		}
		ArrayList<User> u = DBOperation.getInstance().QueryUserByEmail(email);
		if (!u.isEmpty()) {
			map.put("registerWarning", "该邮箱已经注册过了！");
			return "register/register";
		}
		u = DBOperation.getInstance().QueryUserByUserName(nickname);
		if (!u.isEmpty()) {
			map.put("registerWarning", "该昵称已经有人使用！");
			return "register/register";
		}
		DBOperation.getInstance().InsertUser(nickname, email, password);
		this.userName = nickname;
		map.put("userName", getInstance().userName);
		return "main/main";
	}
	@RequestMapping("logging.ha")
	public String logging(String email, String password, Map<String, String> map) {
		if (email.length() > 64) {
			map.put("loginWarning", "您的邮箱太长！");
			return "login/login";
		}
		ArrayList<User> u = DBOperation.getInstance().QueryUserByEmail(email);
		if (u.isEmpty()) {
 			map.put("loginWarning", "账号不存在！");
			return "login/login";
		}
		if (password.length() > 16 || password.length() < 6) {
			map.put("loginWarning", "密码长度应在6到16位之间！");
			return "login/login";
	 	}
		User user = u.get(0);
		if (!password.equals(user.getPassWord())) {
			map.put("loginWarning", "密码错误！");
			return "login/login";
		}
